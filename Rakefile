...
require_or_fail('jeweler', 'Jeweler (or a dependency) not available. Install it with: gem install jeweler') do
  Jeweler::Tasks.new do |gem|
    ...
    gem.description = %Q{A software model in Ruby for the greenhouse gas emissions of a meeting}
    gem.email = "andy@rossmeissl.net"
    gem.homepage = "http://github.com/brighterplanet/meeting"
    ...
    gem.add_runtime_dependency 'earth', '~>0.3.1' unless ENV['LOCAL_EARTH']
  end
...
require_or_fail 'emitter', 'Emitter gem not found, emitter tasks unavailable' do
  ...
  Emitter::Tasks.new.define('meeting')
end
